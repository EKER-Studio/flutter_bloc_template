...
if len(view_box) != 4:
    raise SystemExit(
        f"❌ Invalid viewBox: {view_box_raw!r}\n"
        "   Expected: 0 -960 960 960"
    )
...
if (x, y, width, height) != (0, -960, 960, 960):
    raise SystemExit(
        "❌ Does not match standard Material Icons viewBox.\n"
...
if not children:
    raise SystemExit("❌ SVG does not contain any elements.")
